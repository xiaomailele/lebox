...
#      New B instructions use --secure-input --authorization-ui: user_code is shown only in a private HTML page.
#      Legacy --pair/device_flow remains compatibility-only and is never generated by the secret-free entry.
...
def private_access_token(verified_agent, repo, tools, client_id, token=None):
    """Reuse the release's audited owner-private, non-Git state layout; no credentials in output."""
    import pathlib, types
    module = types.ModuleType('lebox_verified_state_helpers'); module.__file__ = verified_agent
    # The caller has just downloaded AND verified this exact immutable agent.py against trusted release pins.
    exec(compile(pathlib.Path(verified_agent).read_bytes(), verified_agent, 'exec'), module.__dict__)
    scope = {'repository': repo, 'tools': tools, 'client_id': client_id}
    try:
        home = module.persistent_state_home(pathlib.Path.cwd())
        root = module._state_no_links(home / 'github-credentials')
        root.mkdir(mode=0o700, exist_ok=True); os.chmod(root, 0o700)
        filename = hashlib.sha256(module._state_canonical(scope)).hexdigest() + '.json'
        path = module._state_no_links(root / filename)
        with module._state_lock(root):
            if token is None:
                if not path.exists(): return ''
                value = module._state_json(module._state_read(path))
                if set(value) != {'format', 'scope', 'token'} or value['format'] != 'lebox-private-auth-v1' or value['scope'] != scope:
                    stop('AUTH_PRIVATE_SCOPE_MISMATCH', '保留原私有文件，不替换原身份')
                token = value['token']
            elif isinstance(token, str) and 8 <= len(token) <= 4096 and not any(x.isspace() for x in token):
                module._state_write(path, module._state_canonical({'format': 'lebox-private-auth-v1', 'scope': scope, 'token': token}))
            if not isinstance(token, str) or not 8 <= len(token) <= 4096 or any(x.isspace() for x in token):
                stop('AUTH_PRIVATE_TOKEN_INVALID', '不输出私有文件内容')
            return token
    except (OSError, module.StateLocationError):
        stop('AUTH_PRIVATE_STATE_UNAVAILABLE', '原凭据目录不可安全读取或保存；保留原身份，不写入 Git 或聊天')


def device_flow_ui(client_id, repo):
    """User code only in a private, explicitly presented file; bearer/device_code only in memory."""
    import html, pathlib
    if not re.fullmatch(r'[A-Za-z0-9_.-]{1,100}', client_id):
        stop('AUTH_CLIENT_ID_REQUIRED', '请从桌面端重新生成包含公开应用 ID 的 B 指令')
    home = pathlib.Path.home()
    if home.is_symlink() or any((p / '.git').exists() for p in (home, *home.parents)):
        stop('AUTH_PRIVATE_DIRECTORY_REQUIRED', '必须有仓库之外的私有用户目录；不要把授权页写入 Git')
    s = gh_form('/login/device/code', {'client_id': client_id})
    if not isinstance(s, dict) or s.get('verification_uri') != 'https://github.com/login/device' \
            or not isinstance(s.get('device_code'), str) or not 8 <= len(s['device_code']) <= 512 \
            or not isinstance(s.get('user_code'), str) or not re.fullmatch(r'[A-Z0-9-]{6,32}', s['user_code']) \
            or type(s.get('expires_in')) is not int or not 30 <= s['expires_in'] <= 3600 \
            or type(s.get('interval', 5)) is not int or not 1 <= s.get('interval', 5) <= 60:
        stop('AUTH_DEVICE_FLOW_UNAVAILABLE', 'GitHub 未提供有效授权；请确认应用启用了 Device Flow，不要把凭据发到聊天')
    lifetime = min(s['expires_in'], 900); interval = s.get('interval', 5)
    with tempfile.TemporaryDirectory(prefix='.shuncodex-authorization-', dir=home) as directory:
        os.chmod(directory, 0o700)
        page = pathlib.Path(directory) / 'authorize.html'
        markup = '''<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width">
<title>ShunCodex · GitHub 授权</title><style>body{background:#101828;color:#eef2ff;font:18px system-ui;margin:0;padding:48px 24px}main{max-width:620px;margin:auto}small{color:#a8b5cc}h1{font-size:30px}code{display:block;font-size:36px;letter-spacing:.16em;background:#26344e;padding:24px;text-align:center;border-radius:12px}a{color:#a6c8ff}p{line-height:1.65}.note{border-left:3px solid #91baff;padding-left:16px}</style><main>
<small>SHUNCODEX / GITHUB 官方设备授权</small><h1>只需在 GitHub 确认一次</h1>
<p>目标：<b>REPOSITORY</b><br>公开应用 ID：<small>CLIENT</small></p>
<p>在浏览器打开 <a href="https://github.com/login/device" target="_blank" rel="noopener noreferrer">https://github.com/login/device</a>，输入下面的短码。核对 GitHub 显示的应用和仓库权限，再点击授权。</p>
<code id="code">USERCODE</code><p id="expiry">有效期不超过 MINUTES 分钟；拒绝或过期后程序会停止，不会自动换码。</p>
<p class="note">不要把此短码或任何凭据发到聊天。此页不包含访问 Token、device_code 或恢复钥匙。GitHub 授权后仍须通过仓库范围校验和项目访问控制，才会建立连接。</p></main>
<script>setTimeout(()=>{document.getElementById('code').textContent='已过期';document.getElementById('expiry').textContent='请关闭此页；不要重复使用旧短码。'},Math.max(0,EXPIRES_AT-Date.now()))</script></html>'''
        markup = markup.replace('REPOSITORY', html.escape(repo)).replace('CLIENT', html.escape(client_id)) \
            .replace('USERCODE', html.escape(s['user_code'])).replace('MINUTES', str((lifetime + 59) // 60)).replace('EXPIRES_AT', str(int((time.time() + lifetime) * 1000)))
        fd = os.open(page, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
        with os.fdopen(fd, 'w', encoding='utf-8') as stream: stream.write(markup)
        print('AUTHORIZATION_UI_FILE=' + str(page), flush=True)
        print('WAITING_FOR_GITHUB: 仅用安全文件展示工具打开授权页；不要在聊天中转述页面短码。', flush=True)
        deadline = time.monotonic() + lifetime
        while time.monotonic() < deadline:
            time.sleep(min(interval, max(0, deadline - time.monotonic())))
            if time.monotonic() >= deadline: break
            answer = gh_form('/login/oauth/access_token', {'client_id': client_id, 'device_code': s['device_code'],
                             'grant_type': 'urn:ietf:params:oauth:grant-type:device_code'})
            if not isinstance(answer, dict): stop('AUTH_INVALID_RESPONSE')
            token = answer.get('access_token')
            if isinstance(token, str) and 8 <= len(token) <= 4096 and not any(x.isspace() for x in token):
                return token
            error = answer.get('error')
            if error == 'authorization_pending': continue
            if error == 'slow_down': interval = min(60, interval + 5); continue
            stop('AUTH_NOT_GRANTED', '授权被拒绝、已过期或不可用；未生成新码，请检查 GitHub 授权页')
        stop('AUTH_EXPIRED', '未收到 GitHub 授权；不会自动申请新码')
...
def check_scope(token, repo, tools):
    request = lambda path: api(path, token)
...
def main():
    ...
    p.add_argument('--secure-input', action='store_true', help='secret-free entry: reuse protected LEBOX_TOKEN or the explicit private authorization UI')
    p.add_argument('--authorization-ui', action='store_true', help='caller confirms a private HTML-file presentation tool is available; never relay its code into chat')
    ...
    with tempfile.TemporaryDirectory(prefix='lebox-verified-') as directory:
        ...
        token = ''
        if a.secure_input:
            if a.recover or a.pair:
                stop('SECURE_INPUT_CONFLICT', '不要把恢复钥匙或设备码加入无秘密入口')
            token = os.environ.get('LEBOX_TOKEN', '')
            cached = not token and bool(a.client_id)
            if cached:
                token = private_access_token(agent, a.repo, tools, a.client_id)
                if token:
                    status, _ = api('/user', token)
                    if status == 401: token = ''
                    elif status != 200: stop('AUTH_CURRENT_CREDENTIAL_UNVERIFIED', '网络或 GitHub 权限状态未确认；保留原凭据，不自动换授权')
            new_token = not token
            if not token:
                if not a.authorization_ui:
                    stop('SAFE_AUTHORIZATION_UI_REQUIRED', '平台必须支持私有 HTML 文件展示与后台进程；否则停止，不要求在聊天提供 Token 或短码')
                token = device_flow_ui(a.client_id, a.repo)
            check_scope(token, a.repo, tools)
            if new_token: private_access_token(agent, a.repo, tools, a.client_id, token)
            os.environ['LEBOX_AUTH_SOURCE'] = 'device-flow'
            os.environ['LEBOX_SECRET_FREE'] = '1'
            os.environ['LEBOX_RECOVERY_DONE'] = '1'
        elif a.recover:
